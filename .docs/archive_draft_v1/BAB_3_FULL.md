...
5. **Manipulasi Antarmuka Pengguna (DOM Manipulation)**
   Peladen mengembalikan hasil prediksi ke ekstensi Chrome. Apabila label yang dikembalikan adalah "Spam" dan skor keyakinannya melampaui ambang batas (threshold) yang ditetapkan, Content Script dari ekstensi akan mencari elemen komentar yang bersangkutan dan menerapkan penyembunyian melalui injeksi properti Cascading Style Sheets (CSS) sesuai mode yang dipilih pengguna. Sistem menyediakan dua mode penyembunyian. Pada mode Redupkan (dim), komentar tetap berada di dalam DOM namun ditampilkan dengan transparansi rendah (opacity 15%) disertai badge persentase keyakinan yang dapat diklik untuk memunculkan komentar kembali. Pada mode Hilangkan (remove), komentar disembunyikan sepenuhnya dari tampilan melalui properti display: none. Secara bawaan (default), sistem menggunakan mode Redupkan agar pengguna tetap dapat meninjau komentar yang disembunyikan apabila terjadi kesalahan klasifikasi.
