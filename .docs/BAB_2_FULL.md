...
Meskipun algoritma SVM secara native menghasilkan keputusan klasifikasi berdasarkan posisi suatu titik data terhadap hyperplane (apakah berada di sisi positif atau negatif), nilai keluaran fungsi keputusan (decision function) tersebut bukan merupakan probabilitas, melainkan sekadar jarak bertanda (signed distance) dari titik data terhadap bidang pemisah. Nilai ini tidak memiliki batas atas atau bawah yang jelas, sehingga sulit diinterpretasikan sebagai tingkat keyakinan (confidence) yang dapat dipahami secara intuitif oleh pengguna.

Untuk mengonversi jarak tersebut menjadi nilai probabilitas antara 0 dan 1, digunakan teknik Platt Scaling, yaitu metode yang melatih sebuah model regresi logistik sederhana di atas nilai keluaran decision function SVM. Melalui proses ini, jarak mentah terhadap hyperplane dipetakan ke dalam rentang probabilitas menggunakan fungsi sigmoid, sehingga menghasilkan skor kepercayaan yang dapat diinterpretasikan sebagai tingkat keyakinan model terhadap suatu prediksi. Proses kalibrasi ini dilakukan melalui cross-validation internal pada data latih, terpisah dari proses pelatihan hyperplane utama, untuk menghindari estimasi probabilitas yang bias.

Skor probabilitas hasil Platt Scaling inilah yang menjadi dasar dari mekanisme confidence threshold pada sistem yang dibangun, di mana pengguna dapat menyesuaikan ambang batas keyakinan minimum sebelum sebuah komentar dianggap sebagai spam dan disembunyikan.

Sebagai rangkuman, dapat dinyatakan bahwa estimasi probabilitas melalui Platt Scaling menjadi jembatan yang menghubungkan hasil klasifikasi biner SVM dengan kebutuhan sistem akan skor kepercayaan yang dapat disesuaikan pengguna, sehingga keputusan penyembunyian komentar tidak bersifat kaku, melainkan dapat dikendalikan sesuai tingkat toleransi masing-masing pengguna terhadap risiko false positive maupun false negative.
...
## 2.11 Web Scraping dan Analisis Heuristik

Dalam pengembangan kecerdasan buatan, ketersediaan himpunan data (dataset) yang melimpah dan relevan menjadi syarat mutlak. Web scraping adalah teknik mengekstrak data dalam jumlah besar secara cepat dan otomatis dari halaman web (Koprawi & Putra, 2023). Teknik ini dilakukan dengan mengirimkan permintaan protokol jaringan ke peladen situs web, kemudian memuat struktur kode HTML secara utuh, dan mem-parsing elemen-elemen spesifik untuk mendapatkan informasi yang dibutuhkan. Pada arsitektur sistem ini, web scraping berjalan secara asynchronous melalui runtime environment Node.js untuk mengekstraksi komentar-komentar pada platform YouTube tanpa mengganggu eksekusi proses lainnya.

Lebih lanjut, untuk memastikan tingkat validitas data yang ditarik, teknik scraping ini dikombinasikan dengan metode analisis heuristik. Dalam konteks ilmu komputer, pendekatan heuristik merupakan teknik pemecahan masalah berbasis aturan praktis (rule-based) yang memanfaatkan pengetahuan domain untuk menghasilkan keputusan secara efisien, meskipun tidak selalu menjamin solusi yang optimal (Kaddoura dkk., 2022). Dalam penelitian ini, heuristik diimplementasikan sebagai algoritma penyaringan (filter) tahap pertama yang memberikan parameter pembobotan (scoring) otomatis pada hasil ekstraksi. Sistem akan mendeteksi kepadatan anomali karakter (seperti manipulasi Unicode) atau kata kunci spesifik (keywords), sehingga proses pengumpulan data latih promosi perjudian menjadi jauh lebih presisi dan terstruktur sebelum diteruskan ke model pelatihan SVM.

Ringkasnya, kombinasi web scraping dan analisis heuristik dalam penelitian ini tidak hanya berfungsi mengumpulkan data secara otomatis dalam jumlah besar, tetapi juga menyaring kualitasnya sejak tahap awal, sehingga dataset yang dihasilkan lebih relevan dan minim noise sebelum digunakan untuk melatih model klasifikasi.
