...
Penjelasan naratif tiap perubahan (kenapa, apa yang diubah, dampaknya) ada di **[`docs/riwayat-dataset.md`](../../docs/riwayat-dataset.md)** — dokumen ini fokus ke pemetaan versi ↔ commit ↔ file biner saja.
...
| **4** | *(tidak ada)* | — | 2026-06-18 | 1785 / 2513 / 4298 | — | 96.74% | **Tidak ada checkpoint Git terpisah** — preprocessing Step 2b ditambahkan & rescue filter diperbaiki, tapi model tidak sempat di-commit sebelum lanjut ke Versi 5 di hari yang sama. Angka dari `docs/riwayat-dataset.md`. |
| **5** | *(tidak ada)* | — | 2026-06-18 | 1785 / 2513 / 4298 | — | 99.53% | **Tidak ada checkpoint Git terpisah** — Step 5b brand canonicalization & Step 2c ditambahkan, dataset sama dengan V4. Angka dari `docs/riwayat-dataset.md`. |
...
| 8b | [`v8b/`](v8b/) | `c46c2d4` | 2026-06-19 | 2268 / 2864 / 5132 | 6360 | 96.95% | Commit terakhir sebelum sesi audit ini — total baris sudah 5132 sesuai `docs/riwayat-dataset.md`, tapi split spam/non_spam sedikit beda dari tabel V8 (2274/2858) karena ada entri masuk lewat endpoint `/report` tanpa full rebuild. |
| 9 | [`v9/`](v9/) | `d932f41`/`349c813` | 2026-06-30 | 2271 / 2861 / 5132 | 6375 | 97.57% | Dataset di-rebuild bersih dari `prepare_dataset.py` (menghilangkan ketidaksinkronan v8b). Hybrid rules (`ENABLE_HYBRID_RULES`) dimatikan setelah ablation study membuktikan menurunkan akurasi — lihat [`docs/eksperimen.md`](../../docs/eksperimen.md#aturan-heuristik-tambahan). **Disusul v10 di hari yang sama** setelah error analysis lanjutan. |
| 10 | [`v10/`](v10/) | `9490089` | 2026-06-30 | 2271 / 2861 / 5132*** | 6378 | 97.57% (hard test set: 97.04%, naik dari 92.91% di v9) | Error analysis sistematis pada hard test set: 3 contoh "kata laporan tenggelam oleh brand" dipindah dari eval ke training, 3 fragmen terlalu pendek dibuang dari eval. Detail di [`docs/riwayat-dataset.md` Versi 9](../../docs/riwayat-dataset.md#versi-9--2026-06-30). |
...
| 12 | [`v12/`](v12/) | *(belum di-commit)* | 2026-06-30 | 2332 / 4177 / 6509 | 8006 | 97.39% (hard test set: 97.04%) | 61 entri kontaminasi (3 kampanye spam tersamar: Mantulhoki, Hoki777, 4rabet/Anru33 — termasuk 8 yang sudah ada sejak sebelum sesi ini) direlabel dari non_spam ke spam, bukan sekadar dihapus. Akurasi turun dari v11 secara sengaja — v11 sebagian menghafal label salah. Sanity check: brand yang dibersihkan sekarang terdeteksi spam 99.6–100%. Detail di [`docs/riwayat-dataset.md` Versi 11](../../docs/riwayat-dataset.md#versi-11--2026-06-30). |
| 13 | [`v13/`](v13/) | *(belum di-commit)* | 2026-06-30 | 2332 / 4358 / 6690 | 8342 | 97.46% (hard test set: 98.52%, rekor tertinggi) | Scrape ulang 2 video relevan dengan `order=time` + `TARGET_COUNT` lebih tinggi (bukan video baru) untuk menyisir dead zone scraper (skor 10-29) yang biasa dibuang. 181 komentar non-spam bertema judi ditemukan dan ditambahkan. Kasus FN "PBB4D" kini benar masuk training tapi model TETAP salah (99.18% spam) — bukti empiris keterbatasan struktural SVM linear, bukan kurang data. Detail di [`docs/riwayat-dataset.md` Versi 12](../../docs/riwayat-dataset.md#versi-12--2026-06-30). |
| **14** | [`v14/`](v14/) | `5b93f73` | 2026-07-04 | 2332 / 4358 / 6690 (sama dengan v13) | 8342 | **97.53%** | **Model produksi saat ini.** Tidak ada perubahan dataset/parameter dari v13 — murni retrain ulang (`train.py`) saat menambahkan metodologi evaluasi 5-fold CV & analisis dampak stemming. Hash biner beda dari v13 meski konfigurasi identik (kemungkinan non-determinisme kecil di estimasi probabilitas SVM). Diarsipkan agar jejak audit tidak bolong seperti kasus Versi 4 & 5. |

\* Versi 1 dianggap tidak realistis karena non-spam sintetis terlalu seragam — lihat [`docs/riwayat-dataset.md` Versi 1](../../docs/riwayat-dataset.md#versi-1--sebelum-2026-06-16) dan [docs/evaluasi.md](../../docs/evaluasi.md).
\** Angka Versi 3 diambil dari tabel perbandingan di `docs/riwayat-dataset.md` Versi 4 (kolom "Versi 3") — dataset Versi 3 tidak punya tabel evaluasi independen di bagiannya sendiri.
\*** Total baris sama dengan v9, tapi komposisi beda — 3 baris diganti (lihat `docs/riwayat-dataset.md` Versi 9).

---

## Kenapa Ada Lubang di Versi 4 & 5?

Proyek ini dikembangkan dengan retrain manual yang sering (`python src/prepare_dataset.py && python src/train.py`), tapi tidak setiap retrain langsung di-`git commit`. Versi 4 dan 5 dikerjakan di hari yang sama dengan Versi 3 dan 6 (2026-06-18) — perubahannya tercatat lengkap secara naratif di `docs/riwayat-dataset.md` (termasuk metrik evaluasi), tapi file `.joblib` hasil retrain-nya tertimpa oleh retrain berikutnya sebelum sempat di-commit ke Git.
...
`docs/riwayat-dataset.md` mendokumentasikan Versi 8 sebagai satu kesatuan (2274 spam / 2858 non_spam), tapi di Git ada dua commit terpisah yang membungkus proses itu: `2636549` (commit eksplisit "Versi 8", non_spam masih 2799) dan `c46c2d4` (commit berikutnya, "expand dataset... fix report persistence", non_spam naik ke 2864). Selisih ini kemungkinan dari entri yang masuk lewat endpoint `/report` (dev mode) antara dua commit tersebut. Keduanya diarsipkan terpisah (`v8a`, `v8b`) supaya histori tetap akurat — daripada dipaksa jadi satu versi yang menyamarkan perbedaan nyata di datanya.
